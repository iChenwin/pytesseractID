...
def tesseractID(path):
    ...
    # imgcopy = imgcopy * 1.5 + imgMean * 1.6
    # imgcopy = imgcopy / 255
    ...
    texts = tesseractTextImgs(reversed(textImgs))
    texts.insert(0, IDstring)
    for text in texts:
        print "--" + text
...
def tesseractImg(imgs):
    for img in imgs:
        result = pytesseract.image_to_string(Image.fromarray(img), lang='eng', config="-c tessedit_char_whitelist=0123456789X")
...
#tesseract识别
def tesseractTextImgs(imgs):
    texts = []
    for idx, img in enumerate(imgs):
        result = pytesseract.image_to_string(Image.fromarray(img), lang='chi_sim+eng')#, config="-c tessedit_char_whitelist=0123456789X")
        string = re.sub("[\s+\.\!\/_,$%^*(+\"\']+|[+——！，。？、~@#￥%……&*（）“”=:-`′-]+".decode("utf8"), "".decode("utf8"), result)    #出去字符串中的标点符号

        if string != "":
            texts.insert(idx, string)
